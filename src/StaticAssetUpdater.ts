...
export class StaticAssetUpdater {
  ...
  private async findAssetsToUpdate(
    ...
    assetUpdates: Asset[];
    latestVersions: Record<string, CdnPackage>;
  }> {
    ...
    const latestVersions: Record<string, CdnPackage> = {};
    for (const asset of assets) {
      const client = this.getClient(asset.cdn);
      if (client) {
        const latest = await client.getLatestVersion(asset.name);
        if (latest?.version && !this.ignoreAsset(asset, latest.version)) {
          const key = StaticAssetUpdater.getKey(asset);
  ...
  private async createPullRequest(
    ...
    const firstLine = releaseNotesUrl
      ? `Updates ${asset.name} to version [\`${asset.version}\`](${releaseNotesUrl}).`
      : `Updates ${asset.name} to version \`${asset.version}\`.`;
  ...
  private async applyAssetUpdate(
    ...
    // Get the commit that is currently checked out and the root of the repository
    // so that the file changes can be committed via the GitHub API.
    ...
    // Compute the updates in memory; the files are not written to disk by
    // the action as the commit is created via the GitHub API so that it has a
    // verified GPG signature. The committed files are fetched back into the local
    // working tree afterwards so that it matches exactly what was committed.
    for (const file in fileAssetMap) {
      ...
      if (dirty) {
        const relativePath = path
          ...
          .split(path.sep)
          .join('/');
        fileChanges.push({ path: relativePath, content });
      }
    }
    ...
    if (await this.branchExists(octokit, owner, repo, branch)) {
    ...
    const commitOid = await this.commitFileChanges(octokit, {
      branch,
      changes: fileChanges,
    ...
    core.info(
      `Committed ${assetUpdate.name} update to git (${commitOid.substring(0, 7)})`
    );
  ...
  private async commitFileChanges(
    octokit: Octokit,
    change: {
      branch: string;
    ...
    const input = {
      ...
      fileChanges: {
        additions: change.changes.map((c) => ({
          path: c.path,
