...
function registerCommitInterceptors(): void {
  ...
  agent
    ...
    .reply(
      201,
      { ref: 'refs/heads/branch', object: { sha: commitOid } },
      { headers: jsonHeaders }
    )
    .persist();
...
export function getCapturedCommit(branch?: string): CapturedCommit | undefined {
  if (branch) {
    return capturedCommits.find((commit) => commit.branch === branch);
  }
  return capturedCommits[capturedCommits.length - 1];
}
